...
class RNN(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, inertia = 0.5, k_action = 1, max_size = 20):
        ...
        self.h2p1_r2 = nn.Parameter(torch.randn(hidden_size, 2 * max_size + 8) * 0.01 * np.sqrt(2.0 / (hidden_size + 2 * max_size + 8)))
        self.h2p2_r2 = nn.Parameter(torch.randn(hidden_size, 2 * max_size + 8) * 0.01 * np.sqrt(2.0 / (hidden_size + 2 * max_size + 8)))
        self.h2p3_r2 = nn.Parameter(torch.randn(hidden_size, 2 * max_size + 8) * 0.01 * np.sqrt(2.0 / (hidden_size + 2 * max_size + 8)))
        ...
        self.bp1_r2 = nn.Parameter(torch.zeros(1, 2 * max_size + 8))
        self.bp2_r2 = nn.Parameter(torch.zeros(1, 2 * max_size + 8))
        self.bp3_r2 = nn.Parameter(torch.zeros(1, 2 * max_size + 8))
