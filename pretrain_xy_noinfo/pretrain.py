...
class PretrainGame(Game): 
    ...
    def __init__(self, e = 0.2, holes = 3, grid_size = 8, max_size = 20, random_seed = 0, set_reward = 0, time_limit = 100, input_type = 1, batchsize = 1,
        ...
        self.net = RNN(9, 512, 4, k_action = k_action, k_internal = k_internal, k_stim = k_stim).cuda(gpu)
    ...
    def experiment(self, low = 10 , high = 100, e = 0.1, batchsize = 4, size_range = []):   
        ...
        for i in range(batchsize):
            ...
            while self.t <steps.item():
                ...
                # all flatten
                self.input = torch.FloatTensor(self.visible_state).resize(9).cuda(gpu)
                actions.append(self.action_)
                inputs.append(self.input) 
                targets.append(torch.FloatTensor([pos0[0], pos0[1]]).cuda(gpu))
                ...
                if self.grid.grid[pos1] < 0:
                    ...
                    self.agent.pos = pos
#                     self.t += 1
#                     actions.append(torch.eye(4)[action].resize(4).cuda(gpu))
#                     inputs.append(self.input) 
#                     targets.append(self.placefield().float().cuda(gpu))
...
#                 {'params': self.net.a2h, 'lr': lr_rate, 'weight_decay':0},    
        ...
        for epochs in range (50):
                ...
                loss_predict = sum([self.net.distance(predict, target, batchsize) for predict,target in zip(predicts, Targets)]) 
    ...
    def fulltrain(self, lr_rate = 1e-4, decay = 1e-6, trials = 100, low = 10, high = 100, batchsize = 4, size_range = np.arange(10, 20, 1), Loss = []):
        self.Loss = 0
        # start to experiment
        for i in range(trials):
            ...
            if i%50 == 0 and i>0:
                ...
                Loss.append(self.Loss.cpu().data.numpy())
                self.Loss = 0   
