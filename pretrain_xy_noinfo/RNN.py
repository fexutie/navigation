...
class RNN(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, inertia = 0.5, k_action = 1, k_internal = 1, k_stim = 1):
        ...
        self.k_stim = k_stim
        self.i2h = nn.Parameter(torch.randn(input_size, hidden_size) * 10 *  self.k_stim * np.sqrt(2.0/(input_size + hidden_size)))
        ...
        self.h2pr0 = nn.Parameter(torch.randn(hidden_size, 2) * 0.01 * np.sqrt(2.0/(hidden_size + 2)))
        self.h2pr1 = nn.Parameter(torch.randn(hidden_size, 2) * 0.01 * np.sqrt(2.0/(hidden_size + 2)))
        ...
        self.a2h = nn.Parameter(torch.randn(output_size, hidden_size) * self.k_action * np.sqrt(2.0/(hidden_size + output_size)))
        self.bpr0 = nn.Parameter(torch.zeros(1, 2))
        self.bpr1 = nn.Parameter(torch.zeros(1, 2))
    ...
    def forward_sequence(self, inputs, hidden0, actions, reward, control = 0):
        ...
        for input_, action in zip(inputs, actions):  
            hidden_ = F.tanh(input_.matmul(self.i2h) + hidden.matmul(self.h2h) + action.matmul(self.a2h) + reward.matmul(self.r2h) + self.bh)
            hidden = torch.mul((1 - self.r), hidden_) + torch.mul(self.r, hidden) 
            if control == 0:
    ...
    @ staticmethod
    def distance(predict, target, batch_size):
        return 1e-3 * torch.mean((target.view(batch_size,-1) - predict.view(batch_size,-1)) ** 2)
