...
class PretrainTest():
    def __init__(self, weight_write, holes = 0, inputs_type = (1, 0), k_action = 1, k_internal = 1, k_stim = 1):
        ...
        k_action = k_action, k_internal = k_internal, k_stim = k_stim)
...
#       need to take the state dict as a new dict for updating 
    ...
    def pretrain(self, trial, weight = None, lr = 1e-5, decay = 1e-6, pretrain = True, Loss = []):
        ...
        if pretrain == True:
            ...
            self.pregame.fulltrain(lr_rate = lr, decay = decay, trials = int(1e3), batchsize = 4, Loss = Loss)
            print ('pretrain end', torch.norm(self.pregame.net.h2h),\
               np.max(self.pregame.net.h2h.eig()[0].cpu().data.numpy()[:,0]), \
               np.max(self.pregame.net.h2h.eig()[0].cpu().data.numpy()[:,1]))
...
#             prec1 = np.mean(decodetest(self.game, reward_control= 0, epsilon = 1)[0] + decodetest(self.game, reward_control =  1,  epsilon = 1, size = size)[0])
    ...
    def qlearn(self, weight_read, weight_write, iterations = 5, save = True, size_train = np.arange(10, 51, 10), \
        ...
        self.game.k_action = k_action
        self.game.k_internal = k_internal
        self.game.k_stim = k_stim
    ...
    def TestAllSizes(self, size_range = np.arange(15, 86, 10), limit_set = 8, test_size = None, k_action = 1, k_internal = 1, k_stim = 1):
        self.game.net.load_state_dict(torch.load(self.weight))
        self.game.k_action = k_action
        self.game.k_internal = k_internal
        self.game.k_stim = k_stim
