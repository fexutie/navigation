...
class RNN(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, inertia = 0.5, k_action = 1, k_internal = 1, max_size = 20):
        ...
        self.h2p = nn.Parameter(torch.randn(hidden_size, 2 * max_size + 8) * 0.01 * np.sqrt(2.0/(hidden_size + 2 * max_size + 8)))
        ...
        self.bp = nn.Parameter(torch.zeros(1, 2 * max_size + 8))
    ...
    def forward_sequence(self, inputs, hidden0, actions, reward, control = 0):
        ...
        for input_, action in zip(inputs, actions):  
            ...
            hidden = torch.mul((1 - self.r), hidden_) + torch.mul(self.r, hidden) 
            predicts.append(hidden.matmul(self.h2p) + self.bp) 
