...
class RNN(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, inertia=0.5, k_action=1, max_size=20):
        ...
        self.h2o = nn.Parameter(
            torch.randn(hidden_size, output_size) * 0.01 * np.sqrt(2.0 / (hidden_size + output_size)))
        self.h2p1 = nn.Parameter(
            torch.randn(hidden_size, 2 * max_size + 8) * 0.01 * np.sqrt(2.0 / (hidden_size + 2 * max_size + 8)))
        self.h2p2 = nn.Parameter(
            torch.randn(hidden_size, 2 * max_size + 8) * 0.01 * np.sqrt(2.0 / (hidden_size + 2 * max_size + 8)))
        self.h2p3 = nn.Parameter(
            torch.randn(hidden_size, 2 * max_size + 8) * 0.01 * np.sqrt(2.0 / (hidden_size + 2 * max_size + 8)))
    ...
    def forward(self, input, hidden, action, reward):
        ...
        hidden_ = torch.tanh(
            input.matmul(self.i2h) + hidden.matmul(self.h2h) + self.k_action * action.matmul(self.a2h) + reward.matmul(
                self.r2h) + self.bh)
    ...
    def forward_sequence(self, inputs, hidden0, actions, reward, control=0):
        ...
        for input_, action in zip(inputs, actions):
            hidden_ = F.tanh(input_.matmul(self.i2h) + hidden.matmul(self.h2h) + self.k_action * action.matmul(
                self.a2h) + reward.matmul(self.r2h) + self.bh)
            hidden = torch.mul((1 - self.r), hidden_) + torch.mul(self.r, hidden)
...
class decoder(nn.Module):
    def __init__(self, hidden_size, output_size):
        ...
        self.h2p = nn.Parameter(
            torch.randn(hidden_size, output_size) * 1 * np.sqrt(2.0 / (hidden_size + 2 * output_size + 8)))
