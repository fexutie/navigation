...
def trajectory(game, pos0, reward_control = 0, init_hidden = True, hidden = torch.zeros(512, 512), size = 19, test = 2, epsilon = 0, limit_set = 8):
    ...
    while not done:
        ...
        Hidden.append(game.hidden.cpu().data.numpy().squeeze())
        dH.append(torch.norm(game.hidden.cpu() - hidden0.cpu()))
        Pos.append(game.agent.pos)
        Action.append(np.argmax(game.action.cpu().data.numpy()))
        State.append(state)
        hidden0 = game.hidden.clone()
...
#     print (game.visible_state)
    ...
    for iters, stim in enumerate(Stim):
        ...
        Hidden.append(game.hidden.clone().cpu().data.numpy().squeeze()) # need copy , avoid same adress 
        if iters>0:
            dH.append(game.hidden - hidden0)
        Action.append(action)
        hidden0 = game.hidden.clone()
    # problem here dh not converge to 0 
    dH = [dh.cpu().data.numpy().squeeze() for dh in dH]
    return (np.array(Y), np.array(X)), np.array(Hidden), np.array(dH), np.array(Action)



class PCA():
    def __init__(self, game, weight, size = 15, reward_control = 0, epsilon = 1):
        ...
        for j in range (2,17):
            for i in range (2,17):
                ...
                for t in range(T_duration):
                    ...
                    self.Hiddens[(j-2) * 15 + (i-2), t] = (self.game.hidden.cpu().data.numpy().squeeze())
                    if done == True:
                        self.game.reset(reward_control = self.reward_control, size = 15)
                        done = False 

    def pca(self, T_duration = None):
        if T_duration == None:
...
def fixpoints(game, weight, hidden0 = torch.randn(1, 512, requires_grad = True), action = 4, stim = torch.zeros(1, 9)):
    pca = PCA(game, weight = weight)
