...
class RNN(nn.Module):
    ...
    def velocity(self, input, hidden, action, reward):
        hidden_ = torch.tanh(input.cpu().matmul(self.i2h.cpu()) + hidden.cpu().matmul(self.h2h.cpu()) + self.k_action * action.cpu().matmul(self.a2h.cpu()) + reward.cpu().matmul(self.r2h.cpu()) + self.bh.cpu())
        return torch.norm((hidden.cpu() - hidden_).view(-1))
