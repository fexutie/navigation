...
class PretrainTest():
    def __init__(self, weight_write, holes = 0, inputs_type = (1, 0), set_reward = [(0.5, 0.25), (0.5, 0.75)]):
        self.pregame = PretrainGame(grid_size = (15, 15), holes = holes, random_seed = 4 , set_reward = set_reward, input_type = inputs_type[0])
        self.game = ValueMaxGame(grid_size = (15, 15), holes = 0, random_seed = 4 , set_reward = set_reward, input_type = inputs_type[1])
...
#       need to take the state dict as a new dict for updating 
    ...
    def qlearn(self, weight_read, weight_write, iterations = 5, save = True, size_train = np.arange(10, 51, 10), k_action = 1.,\
               size_test = [10, 30], train_only = False, test_only = False, noise = 0.3, h2o = True, shuffle = False, randomnize = False):
        ...
        if shuffle == True and randomnize == False:
            self.weight_shuffle()
        if randomnize == True and shuffle == False: 
            self.weight_randomnization()
...
def trajectory(game, pos0, reward_control = 0, init_hidden = True, hidden = torch.zeros(1, 512), size = 19, test = 2, limit_set = 4,
              animate = False, force_actions = []):
...
class ActivityCheck():
    def __init__(self, size = 15, num_pc = 3):
        ...
        self.size = size
        self.num_pc = num_pc
    def activity(self, reward_control = 0, weight = 0, pc = False, vects = None, noise = 0):    
