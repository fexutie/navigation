...
#  decode is binary  
    ...
    def step(self, policy, epsilon = 'default', record = False, test = False, cross = False, train_hidden = False, decode = False):
        ...
        def TD(decode = False):
            realQ,_  = self.net(Variable(torch.FloatTensor(state_t1)).resize(1,9), self.hidden, self.action, self.placefield(self.pos_reward_))
    ...
    def step_force(self, action):
        ...
        pos0 = self.agent.pos
        # network dynamics and decision
        self.values, self.hidden = self.net(torch.FloatTensor(state_t0), self.hidden, self.action, self.placefield(self.pos_reward_))
        # action to state
        self.agent.act(action) 
        self.action = torch.eye(4)[action].resize(1, 4)
        pos1 = self.agent.pos
