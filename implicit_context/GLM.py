...
def Data_record(weight, k_action = 1, epsilon = 0, size = 15, T = 200, seed_num = 1e3, limit_set = 4):
    PC_traces = []
    Hiddens = []
    Poss = []
    Actions = []
    ...
    for i in range(T):
        ...
        Pos, hidden, dh, Action, State, reward = trajectory(game, start, reward_control = c, size = size, \
                                                                  test = 0, limit_set = limit_set, init_hidden = False, hidden = hidden0, epsilon = epsilon)
