...
class RNN(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, inertia = 0.5, k_action = 1, k_internal = 0.8, max_size = 20):
        super(RNN, self).__init__()
        self.hidden_size = hidden_size
        self.output_size = output_size
        self.i2h = nn.Parameter(torch.randn(input_size, hidden_size) * 10 * np.sqrt(2.0/(input_size + hidden_size)))
        self.h2h = nn.Parameter(torch.randn(hidden_size, hidden_size) * k_internal * np.sqrt(2.0/hidden_size))
        ...
        self.a2h = nn.Parameter(torch.randn(4, hidden_size) * k_action * np.sqrt(2.0/(hidden_size + 4)))
