...
#  decode is binary  
    ...
    def step_empty(self, stim = torch.zeros(1, 9), action_ = 0, epsilon = 'default', T0 = 100, T_total = 400, T_duration = 60, open_loop = True):
        self.t += 1
        # set e in any case 
        if epsilon !=  'default':
            self.e = epsilon
        # T0 is tart time for stimulus
        if self.t < T0+T_duration and self.t>T0:
            state = stim
        else:
    ...
    def episode(self, epochs = 10, epsilon = 'default', reward_control = None, size_range = (10, 20), train_hidden = False, test = False, decode = False):   
        if reward_control == None:
            self.reset(reward_control = np.random.randint(len(self.set_reward)), size_range = size_range)
        else:
            self.reset(reward_control = reward_control, size_range = size_range)
        done = False
...
#         print('start episode', process.memory_info().rss) 
    ...
    def experiment(self, rls_q, rls_sl, iterations = 10, epochs = 20, epsilon = 0.5, num_episodes = 100, reward_control = None, train_hidden = False, train_q = True, decode = False, size_range = [10], test = False):
        ...
        for i in range(iterations):
            self.episode(epochs = epochs, epsilon = epsilon, reward_control = reward_control, size_range = size_range, train_hidden = train_hidden, test = test, decode = decode)
