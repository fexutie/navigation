...
class Game():
    ...
    # set limit sizes 
    def reset(self, set_agent = 0, action = True, reward_control = 0, size = None, size_range = np.arange(10, 51, 10), limit_set = 8, test = None):
        """Start a new episode by resetting grid and agent"""
        # reset the reward so that it will not be erased in time 
        if size == None:
            prob = len(size_range) * [1./(len(size_range))]
...
#         print (size)
        ...
        if test!= None:
            radius = test
        self.grid = Grid(n_holes = self.holes, grid_size = (size, size), random_seed = self.seed, set_reward = Set_reward)
