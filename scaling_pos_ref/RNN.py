...
#  Put position input into the network to get perfect informationb
...
class RNN(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, inertia = 0.5, k_action = 0, max_size = 15, forget = 0):
        ...
        self.h2pr0 = nn.Parameter(torch.randn(hidden_size, 2 * max_size + 8) * 0.01 * np.sqrt(2.0/(hidden_size + 2 * max_size + 8)))
        self.h2pr1 = nn.Parameter(torch.randn(hidden_size, 2 * max_size + 8) * 0.01 * np.sqrt(2.0/(hidden_size + 2 * max_size + 8)))
        ...
        self.p2h = nn.Parameter(torch.randn(2 * max_size + 8, hidden_size) * 1 * np.sqrt(2.0/(hidden_size + 2 * max_size + 8)))
        self.bpr0 = nn.Parameter(torch.zeros(1, 2 * max_size + 8))
        self.bpr1 = nn.Parameter(torch.zeros(1, 2 * max_size + 8))
    ...
    def forward(self, input, hidden, action, position, reward):
        # dim should be same except catting dimension
        hidden_ = torch.tanh(input.matmul(self.i2h) + (1 - self.forget) * hidden.matmul(self.h2h) + self.k_action * action.matmul(self.a2h) + reward.matmul(self.r2h) + position.matmul(self.p2h) + self.bh)
        hidden = torch.mul((1 - self.r), hidden_) + torch.mul(self.r, hidden) 
        output = hidden.matmul(self.h2o)+ self.bo
        return output, hidden
    
    def forward_sequence(self, inputs, hidden0, actions, positions, reward, control = 0):
        # dim should be same except catting dimension
        hidden = hidden0
        predicts = []
        hiddens = []
        for input_, action, position in zip(inputs, actions, positions):  
            hidden_ = F.tanh(input_.matmul(self.i2h) + hidden.matmul(self.h2h) + self.k_action * action.matmul(self.a2h) + reward.matmul(self.r2h) +  position.matmul(self.p2h) + self.bh)
            hidden = torch.mul((1 - self.r), hidden_) + torch.mul(self.r, hidden) 
            if control == 0:
                predicts.append(hidden.matmul(self.h2pr0) + self.bpr0)
            else:
                predicts.append(hidden.matmul(self.h2pr1) + self.bpr1) 
            hiddens.append(hidden)
        return predicts, hiddens
