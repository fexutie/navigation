...
class Grid():
    def __init__(self, n_holes = 4, grid_size = GRID_SIZE, random_seed = 0, set_reward = 0, eta = 0):
    ...
    # agent gets its relative position here 
    def visible1(self, pos):
        ...
        if np.sum(visible) != 0 and (x ==2 or x == self.grid_size_x + 1): 
             visible = np.multiply(visible, self.eta * np.random.normal(size = (3, 3)) + y_relative * np.ones((3,3)))
        elif np.sum(visible) != 0 and (y ==2 or y == self.grid_size_y + 1): 
             visible = np.multiply(visible, self.eta * np.random.normal(size = (3, 3)) + x_relative * np.ones((3,3)))
        return visible
...
class Game():
    # 初始化，初始grid和agent
    def __init__(self, grid_size = 8, holes = 4, discount = 0.99, time_limit = 200, random_seed = 0, set_reward = 0, input_type = 0, eta = 0):
        ...
        self.grid = Grid(n_holes = holes, grid_size = grid_size, random_seed = random_seed, set_reward = self.Set_reward, eta = self.eta)
        self.agent = Agent()
        self.History = []
    ...
    # set limit sizes 
    def reset(self, set_agent = 0, action = True, reward_control = 0, size = None, size_range = np.arange(10, 51, 10), prob = 5 * [0.2] , limit_set = 8, test = None):
        ...
        self.grid = Grid(n_holes = self.holes, grid_size = (size, size), random_seed = self.seed, set_reward = self.Set_reward, eta = self.eta)
        self.grid_size = (self.grid.grid_size_y, self.grid.grid_size_x)
        self.grid.grid[2:self.grid.grid_size_y+2,2:self.grid.grid_size_x+2] = 0
