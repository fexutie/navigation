...
class ValueMaxGame(Game): 
    ...
    def __init__(self, e = 0.2, holes = 3, grid_size = 8, random_seed = 0, set_reward = 0, time_limit = 200, input_type = 0, action_control = 1, lam = 0.5, discount = 0.99, alpha = 0.5, eta = 0, Abs = True, forget = 0):
        Game.__init__(self, discount = discount, grid_size = grid_size, holes = holes, time_limit = time_limit, random_seed = random_seed, set_reward = set_reward, input_type = input_type, eta = eta)
        # need to have randomness
        self.e = e
        self.net = RNN(9, 512, 4, k_action = 1, forget = forget)
...
#                 print (i - pos[k])e
    ...
    def placefield_abs(self, pos, status = [0, 0]): 
#         print (pos_, self.set_reward)
#         print (self.grid_size)
        field =np.zeros((2, 19))
#         print (len(field))
        for k in range(2):
            if self.Abs == False:
                x = pos[k] * 19./(self.grid_size[1] + 4)
            else:
                x = pos[k] + self.eta * np.random.randint(2, self.grid_size[k] + 2)
            for i in range(field.shape[1]): 
            # distance generation 
                field[k, i] =  status[k] * ((i- x)/3) ** 2  
          
        # gaussian density, but before exponential to help learning identity mapping input to output
        field = - 0.1 * torch.from_numpy(field).resize(1, 2 * 19).float()
        return field
    # given input as the position, in same form of context reward    
    def maxplay(self, state, status, cross = False):
        # Move according to action: 0=UP, 1=RIGHT, 2=DOWN, 3=LEFT
        # value for four action     
        action0 = self.action.clone()
        if cross == False: 
            self.values, self.hidden = self.net(state, self.hidden, self.action, 
                                self.placefield_abs(self.agent.pos, status = status), self.placefield(self.pos_reward))
        else:
            self.values, self.hidden = self.net(state, self.hidden, self.action, self.placefield_abs(self.agent.pos, status = status), self.placefield(self.pos_reward_))
...
#  decode is binary  
    ...
    def step(self, policy, epsilon = 'default', record = False, test = False, cross = False, train_hidden = False, decode = False, status = [0, 0]):
        if epsilon != 'default': 
            self.e = epsilon  
        self.t += 1
        ...
        action, action0 = policy(Variable(torch.FloatTensor(state_t0)).resize(1,9), status, cross = cross)
        # action to state
        ...
        def TD(decode = False):
            realQ,_  = self.net(Variable(torch.FloatTensor(state_t1)).resize(1,9), self.hidden, self.action, self.placefield_abs(self.agent.pos, status), self.placefield(self.pos_reward))
    ...
    def step_empty(self, stim = torch.zeros(1, 9), action_ = 0, epsilon = 'default', T0 = 100, T = 400, time_stop = 60, open_loop = True):
        ...
        self.values, self.hidden = self.net(torch.FloatTensor(state).resize(1, 9), self.hidden, self.action,  self.placefield(self.agent.pos), self.placefield(self.pos_reward))
        action = self.values.data.cpu().numpy().argmax()
...
#         print('start episode', process.memory_info().rss) 
        for i in range(epochs):
            ...
            for t in range(self.size * 10):
                _, state_t0, reward, done = self.step(self.maxplay, epsilon = epsilon, train_hidden = train_hidden, test = test, decode = decode, status = status)
                if self.agent.pos[0] == 2 or self.agent.pos[0] == self.grid.grid_size_y + 1:
                    status[0] = 1
                if self.agent.pos[1] == 2 or self.agent.pos[1] == self.grid.grid_size_x + 1:
                    status[1] = 1
                if done == True:
                    Done()
            Done()
    ...
    def experiment(self, rls_q, rls_sl, iterations = 10, epochs = 20, epsilon = 0.5, num_episodes = 100, reward_control = None, train_hidden = False, train_q = True, decode = False, size_range = (10, 20), test = False):
        # initialize, might take data during test
        self.trace = []
        self.Qs = []
        self.Hiddens = []
        self.Pos = []
        prob = [1./len(size_range)] * len(size_range)
        for i in range(iterations):
            self.episode(epochs = epochs, epsilon = epsilon, reward_control = reward_control, size_range = size_range, prob = prob, train_hidden = train_hidden, test = test, decode = decode)
...
def Test(game, weights = 0, reward_control = [0], cross = False, size = 15, test = 1, limit_set = 2):
    ...
    for j in np.arange (2 * VISIBLE_RADIUS, size + 2 * VISIBLE_RADIUS, step):
        for i in np.arange (2 * VISIBLE_RADIUS, size + 2 * VISIBLE_RADIUS, step):
            ...
            status = [0, 0]
            while not done:
                _, state, reward, done = game.step(game.maxplay, epsilon = 0.00, test = True, cross = cross, status = status) # Down
                if game.agent.pos[0] == 2 or game.agent.pos[0] == game.grid.grid_size_y + 1:
                    status[0] = 1
                if game.agent.pos[1] == 2 or game.agent.pos[1] == game.grid.grid_size_x + 1:
                    status[1] = 1
            if i<=pos_r[1]:
