...
class PretrainTest():
    def __init__(self, weight_write, holes = 0, inputs_type = (1, 0), eta = 0, Abs = True):
        ...
        self.game = ValueMaxGame(grid_size = (15, 15), holes = 0, random_seed = 4 , set_reward = [(0.5, 0.25), (0.5, 0.75)], \
                                 input_type = inputs_type[1], eta = eta, Abs = Abs)
...
#       need to take the state dict as a new dict for updating , here is only tensor of trained net
    ...
    def decode(self, weight = None, size_range = [15], size_test = [15], epsilon = 0):
        ...
        def precision(size = 15, reward_control = 0):
            ...
            prec0 = np.mean(dist)
            return np.mean(decode[2:-2, 2:-2]/visit[2:-2, 2:-2]), Cor_y, Cor_x
        Prec = 0
        Cory = 0
        Corx = 0
        ...
        for reward_control in [0, 1]:
            ...
            prec, cory, corx =  precision(size = size_test[0], reward_control = reward_control)
            Prec += prec
            Cory += cory
            Corx += corx
        ...
        Cory = Cory/2
        Corx = Corx/2
        print ('decode train finish', Prec, Cory, Corx)
        return Prec, Cory, Corx 
...
def trajectory(game, pos0, reward_control = 0, init_hidden = True, hidden = torch.zeros(512, 512), size = 19, test = 2, time_limit = 8):
    game.reset(set_agent = pos0, reward_control = reward_control, size = size, limit_set = time_limit, test = test)
    done = False
    if init_hidden == True:
...
class PCA():
    def __init__(self, size = 15, reward_control = 0, trial = 0, file = 1, iters1 = 0, iters2 = 9, epsilon = 1):
        ...
        self.weight = 'weights_cpu/rnn_1515tanh512_checkpoint{}'.format(trial)
    ...
    def TimeScale(self, plot = False, T_total = 800, Hiddens = [], noise = 2, iters = 4, Actions = [2, 0, 1, 3], e = 0, T0 = 100, Time_stop = 60, same = True, legend = False, corner = False, open_loop = True, readout_random = False) :
        Attractors = []
        Timescale = []
        Trajectory = []
        Positions = []
...
#                 if np.sum((dh1[T0:]>1e-1))<300:
        ...
        f, pd = signal.periodogram(np.array(Trajectory).ravel(), fs = 1)
#         print  ('fix point', np.mean(distance.cdist(Attractors, Attractors, 'euclidean')))
#         print ('timescale', np.mean(Ts), np.var(Ts)) 
        plt.figure(1)
        plt.plot(f, pd)
        plt.xlim(0.01, 0.1)
        plt.ylim(0 , 100)
