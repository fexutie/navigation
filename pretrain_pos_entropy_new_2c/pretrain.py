...
#         self.action = Variable(torch.eye(4)[Action]).resize(1, 4).cuda()  
    ...
    def train(self, lr_rate, Actions, Inputs, Targets, decay = 1e-6, beta = 1e-2):
        ...
        optimizer = torch.optim.Adam(
                [ 
                {'params': self.net.h2pr0, 'lr': 100 * lr_rate, 'weight_decay':0},
                {'params': self.net.h2pr1, 'lr': 100 * lr_rate, 'weight_decay': 0},
                ...
                {'params': self.net.bpr0, 'lr': 100 * lr_rate},
                {'params': self.net.bpr1, 'lr': 100 * lr_rate},
                ])
        ...
        for epochs in range (50):
            ...
            if reward_control == 0:
                reward_input = torch.stack(
                    [self.placefield_reward(self.Set_reward[0]) for i in range(batchsize)]).squeeze().cuda()
            else:
                reward_input = torch.stack(
                    [self.placefield_reward(self.Set_reward[1]) for i in range(batchsize)]).squeeze().cuda()
            ...
            for p, name in zip([self.net.i2h, self.net.a2h, self.net.h2h, self.net.bh], ['i2h', 'r2h', 'a2h', 'h2h', 'bh', 'h2p', 'bp'] ):
