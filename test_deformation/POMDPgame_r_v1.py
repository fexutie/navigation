...
class Grid():
    def __init__(self, n_holes = 4, grid_size = GRID_SIZE, random_seed = 0, set_reward = 0, ry = 5, rx = 4, train = True):
        ...
        if train == True:
            rx = rx - 1 + np.random.randint(3)
            ry = ry - 2 + np.random.randint(3)
            # return a number for block length
            length = np.random.randint(10, 13)
        else:
            ry, rx = ry, rx
            length = 11
        self.grid[ry : ry + 1, rx : rx + length] = 0.5 * HOLE_VALUE
        # Goal set
        if set_reward == 0:
