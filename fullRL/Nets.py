...
#       need to take the state dict as a new dict for updating 
    ...
    def decode(self, weight = None, size_range = [15], size_test = [15], epsilon = 0):
        ...
        self.game.experiment(rls_q, rls_sl, epochs = 20, epsilon = epsilon, train_hidden = False, train_q = False, size_range = size_range, test = True, decode = True)
    ...
    def qlearn(self, weight_write, weight_read = None, iterations = 5, save = True, size_train = np.arange(10, 51, 10), size_test = [10, 30], train_only = False, test_only = False, noise = 0.3, k_action = 1, h2o = True, shuffle = False):
        if weight_read != None:
        ...
        for n,e in enumerate(e_rate):
            ...
            if test_only == False:
                self.game.experiment(rls_sl, epochs= 10, epsilon = e, size_range = size_train)
                if save == True:
