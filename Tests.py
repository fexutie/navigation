...
def Test(task, game, weight = 0):
    if weight != 0:
        game.net.load_state_dict(torch.load(weight))
    def TestBasic(game, reward_control= 0, size=15, test =0, limit_set = 8, start=[]):
        Rewards = 0
        iters = 0
        error = 0
        step = size // 15 + 1
    ...
    def TestBar(game, reward_control=0, cross=False, size=15, test =0, limit_set = 8, map_set=[], start=[]):
        Rewards = 0
        iters = 0
        error = 0
        step = size // 15 + 1
    ...
    def TestHole(game, reward_control= 0, cross=False, size=15, test =0, limit_set = 8, seed=0, holes = 50):
        Rewards = 0
        iters = 0
        error = 0
        step = size // 15 + 1
    ...
    def TestScale(game, reward_control= 0, cross=False, size = 50, test =0, limit_set = 8):
        Rewards = 0
        iters = 0
        error = 0
        step = size // 15 + 1
    ...
    def TestScale_x(game, reward_control = 0, cross=False, size=15, test =0, limit_set = 8, scale = 3, start=[]):
        Rewards = 0
        iters = 0
        error = 0
        step = size // 15 + 1
    ...
    def TestScale_y(game,reward_control = 0, cross=False, size=15, test = 0, limit_set = 8, scale = 3, start=[]):
        Rewards = 0
        iters = 0
        error = 0
        step = size // 15 + 1
    ...
    def TestImplicit(game, reward_control = 0, cross=False, size=15, test =0, limit_set = 8):
        Rewards = 0
        iters = 0
        error = 0
        step = size // 15 + 1
    ...
    for context in [0, 1]:
        if task == 'basic':
            performance += TestBasic(game, reward_control = context)
        if task == 'hole':
            performance += TestHole(game, reward_control = context, holes=50, seed = 0)
        if task == 'bar':
            performance += TestBar(game, reward_control = context)
        if task == 'scale':
            performance += TestScale(game, size = 15, reward_control = context)
        if task == 'scale_x':
            performance += TestScale_x(game, scale = 3, reward_control = context)
        if task == 'scale_y':
            performance += TestScale_y(game, scale = 3, reward_control = context)
        if task == 'implicit':
            performance += TestImplicit(game, reward_control = context)
