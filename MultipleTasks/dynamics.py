...
def weight_lowrank(net, trial = 300, ranks = [], descend = True, random = True, replace = False):
    W0 = torch.load('weights_pos2/rnn_1515tanh512_checkpoint0')['h2h'].data.numpy()
    Wt = torch.load('weights_pos2/rnn_1515tanh512_checkpoint{}'.format(trial))['h2h'].data.numpy()
    u, s, vh = np.linalg.svd(Wt - W0)
    smat = np.zeros_like(np.diag(s))
    for r in ranks[:-1]:
        smat[r, r] = np.diag(s)[r, r]
#     print (smat)
    Wper = np.dot(smat, vh)
    Wper = np.dot(u, Wper)
    if descend == True:
        W = Wt - Wper
    elif (descend == False and random == True):
        W = W0 + Wper
    elif (descend == False and random == False):
        W = Wper
    elif (descend == False and replace == True):
        W0 = nn.Parameter(torch.randn(hidden_size, hidden_size) * 1.0 * np.sqrt(2.0/hidden_size))
        W = W0 + Wper
    h2h = torch.from_numpy(W)
    net.h2h = nn.Parameter(h2h)
#     print ('norm', np.linalg.norm(Wper))
# use matrix itself instead of the difference 
def weight_lowrank2(net, trial = 300, ranks = [], descend = True):
    W0 = torch.load('weights_pos2/rnn_1515tanh512_checkpoint0')['h2h'].data.numpy()
    Wt = torch.load('weights_pos2/rnn_1515tanh512_checkpoint{}'.format(trial))['h2h'].data.numpy()
    u, s, vh = np.linalg.svd(Wt)
    smat = np.zeros_like(np.diag(s))
    for rank in ranks:
...
def weight_eig(net, trial = 300, ranks = [], descend = True, random = True):
    W0 = torch.load('weights_pos2/rnn_1515tanh512_checkpoint0')['h2h'].data.numpy()
    Wt = torch.load('weights_pos2/rnn_1515tanh512_checkpoint{}'.format(trial))['h2h'].data.numpy()
    vals, vect = np.linalg.eig(Wt)
    smat = np.zeros_like(np.diag(vals))
    for rank in ranks:
        smat[rank, rank] = np.diag(vals)[rank, rank]
    Wp = np.dot(smat, vect)
    Wp = np.dot(np.linalg.inv(vect), Wp)
    print (Wp)
    h2h = torch.from_numpy(Wp)
    net.h2h = nn.Parameter(h2h)
...
class PCA():
    def __init__(self, weight = None, size = 15, reward_control = 0, epsilon = 1):
    ...
    def record(self, T_duration = 50):
        self.Hiddens = np.zeros((225, H, 512))
        for j in range (2,17):
            for i in range (2,17):
...
#         self.vals, self.vect = np.linalg.eig(cov)
    ...
    def pca(self, T_duration = None, lowrank = [], descend = True, random = True, replace = False):
        if lowrank != []:
            weight_lowrank(self.game.net, ranks = lowrank, descend = descend, random = random, replace = replace)
        if T_duration == None:
