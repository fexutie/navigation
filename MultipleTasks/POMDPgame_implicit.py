...
class Grid():
    def __init__(self, n_holes = 4, grid_size = GRID_SIZE, random_seed = 0, set_reward = 0):
...
class GameImplicit():
    ...
    # set limit sizes 
    def reset(self, set_agent = 0, action = True, reward_control = 0, size = None, size_range = np.arange(10, 51, 10), prob = 5 * [0.2] , limit_set = 8, test = None, context = (0.5, 0.25)):
        ...
        if len(self.set_reward) != 0:
            ...
            self.pos_reward_ = Set_reward[0]
#             self.pos_reward = Set_reward[0]
#             self.pos_reward_ = Set_reward[1]
            # select the reward 
    ...
    def visible_state(self):
        ...
        if self.input_type == 0:
            if self.reward_control == 0:
                visible = self.grid.visible(self.agent.pos)
            if self.reward_control == 1: 
                if (self.agent.pos[1] == 2 or self.agent.pos[1] == self.grid_size[1] + 1):
                    visible = self.grid.visible((self.agent.pos[0], (self.grid_size[1] + 3 - self.agent.pos[1])))
                else:
                    visible = self.grid.visible(self.agent.pos)
