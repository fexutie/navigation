...
#       need to take the state dict as a new dict for updating 
        ...
        list_modules = [('h2h', net_dict['h2h']), ('a2h', net_dict['a2h']), ('i2h', net_dict['i2h']), ('r2h', net_dict['r2h']), ('bh', net_dict['bh']), ('h2o', net_dict['h2o']), ('bo', net_dict['bo'])]
    ...
    def weight_shuffle(self):
        I = torch.randperm(512)
        J = torch.randperm(512)
        h2h = self.game.net.h2h.data
        self.game.net.h2h = torch.nn.Parameter(h2h[I][:,J])
        I = torch.randperm(4)
        J = torch.randperm(512)
        a2h = self.game.net.a2h.data
        self.game.net.a2h = torch.nn.Parameter(a2h[I][:,J])
        I = torch.randperm(9)
        J = torch.randperm(512)
        I2h = self.game.net.i2h.data
        self.game.net.i2h = torch.nn.Parameter(I2h[I][:,J])
    ...
    def decode(self, weight = None, size_range = [15], size_test = [15], epsilon = 0):
        ...
        rls_sl = RLS(1e2)
        size = size_range[0]
        def precision(size = size, reward_control = 0):
            dist, decode, visit, entropy = decodetest(self.game, reward_control= reward_control, epsilon = 0, size = size)
            prec0 = np.mean(dist)
            return np.mean(decode/visit), decode/visit, entropy
        Prec = 0
        # iterations are number of turns for decoder update, epochs are how many turns of games at each update  
        Entropy = 0
        Prec_matrix = np.zeros((size_test[0], size_test[0]))
        self.game.experiment(rls_q, rls_sl, iterations = 1, epochs = 200, epsilon = epsilon, train_hidden = False, train_q = False, size_range = size_range, test = True, decode = True) 
        for size in size_test:
            for reward_control in [0, 1]:
                prec, prec_matrix, entropy =  precision(size = size, reward_control = reward_control)
                Prec += prec
    ...
    def decode_sgd(self, weight = None, size_range = [15], size_test = [15], epsilon = 0, reward_control = 0):
        if weight != None:
            self.game.net.load_state_dict(torch.load(weight))
        else:
            self.game.net.load_state_dict(torch.load(self.weight))
        for i in range(3):
            self.game.experiment_decode(iterations = 5, epochs = 50, epsilon = 0.0, reward_control = reward_control, size_range = [15]) 
            dist, decode, visit = decodetest(self.game, reward_control= 0, epsilon = 0, size = 15, sgd = True)
        return dist, decode/visit
    
        
    def qlearn(self, weight_read, weight_write, iterations = 5, save = True, size_train = np.arange(10, 51, 10), size_test = [10, 30], train_only = False, test_only = False, noise = 0.3, k_action = 1, h2o = True, shuffle = False):
        ...
        for n,e in enumerate(e_rate):
            ...
            self.game.net.lam = float((1 + rewards)/2)
            self.game.alpha = float(1 - (1 + rewards)/2)
        return Rewards
    
    def TestAllSizes(self, size_range = np.arange(15, 86, 10), limit_set = 8, test_size = None, cross = False):
        self.game.net.load_state_dict(torch.load(self.weight))
        self.Performance = []
        for test_platform, size in zip(test_size, size_range):
            if test_size == None: 
                test_size = size//25
            else:
                test_size = test_size
            Rewards0 = Test(self.game, reward_control = 0, size = size, test = test_platform, limit_set = limit_set, cross = cross)
            Rewards1 = Test(self.game, reward_control = 1, size = size, test = test_platform, limit_set = limit_set, cross = cross)
...
def trajectory(game, pos0, reward_control = 0, init_hidden = True, hidden = torch.zeros(1, 512), size = 19, test = 2, limit_set = 4, animate = False, force_actions = [], epsilon = 0):
    ...
    if force_actions != []:
        for action in force_actions:
            pos0, state = game.step_force(action) 
    ...
    while not done:
        ...
        if animate == True:
            ...
            grid[game.agent.pos] = 1 # Agent marker faded by health
            history.append((grid, game.t))
    if animate == False:
        return Pos, np.array(Hidden), np.array(dH), np.array(Action), State, reward
    else:
        return Pos, np.array(Hidden), np.array(dH), np.array(Action), State, reward, history
...
class ActivityCheck():
    def __init__(self, size = 15, num_pc = 3):
        ...
        self.size = size
        self.num_pc = num_pc
    def activity(self, reward_control = None, weight = 0, pc = False, vects = None, noise = 0):    
        ...
        if reward_control == None:
            context = Context[np.random.randint(2)] 
            reward_control = np.random.randint(2)
        else:
            context = Context[reward_control]
        for i in range (2, self.size + 2):
            for j in range (2, self.size + 2):
