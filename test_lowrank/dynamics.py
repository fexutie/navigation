...
def weight_lowrank(net, trial = 300, ranks = [], descend = True, random = True):
...
def weight_lowrank2(net, trial = 300, ranks = [], descend = True, random = True):
    W0 = torch.load('weights_pos2/rnn_1515tanh512_checkpoint0')['h2h'].data.numpy()
    Wt = torch.load('weights_pos2/rnn_1515tanh512_checkpoint{}'.format(trial))['h2h'].data.numpy()
...
#         self.vals, self.vect = np.linalg.eig(cov)
    ...
    def pca(self, T_duration = None, lowrank = [], descend = True, random = True):
        if lowrank != []:
            weight_lowrank(self.game.net, ranks = lowrank, descend = descend, random = random)
        if T_duration == None:
