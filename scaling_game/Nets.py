...
class PretrainTest():
    def __init__(self, weight_write, holes = 0, inputs_type = (1, 0), size = 15):
        self.pregame = PretrainGame(grid_size = (size, size), holes = holes, random_seed = 4 , set_reward = [(0.5, 0.25), (0.5, 0.75)], input_type = inputs_type[0])
        self.game = ValueMaxGame(grid_size = (size, size), holes = 0, random_seed = 4 , set_reward = [(0.5, 0.25), (0.5, 0.75)], input_type = inputs_type[1])
...
#       need to take the state dict as a new dict for updating 
    ...
    def decode_rls(self, weight = None, size_range = [15], size_test = [15], epsilon = 0):
        if weight != None:
        ...
        def precision(size = size, reward_control = 0):
            dist, decode, visit = decodetest(self.game, reward_control= reward_control, epsilon = 0, size = size)
            prec0 = np.mean(dist)
        ...
        Prec_matrix = np.zeros((size, size))
...
def trajectory(game, pos0, reward_control = 0, init_hidden = True, hidden = torch.zeros(1, 512), size = 19, test = 2, limit_set = 4, animate = False, epsilon = 0):
...
def animate(history, history_p, states):
    frames = len(history)
    print("Rendering %d frames..." % frames)
    fig = plt.figure(figsize=(6, 2))
    fig_grid1 = fig.add_subplot(131)
    fig_grid2 = fig.add_subplot(132)
    fig_grid3 = fig.add_subplot(133)
    def render_frame(i):
        grid1, time = history[i]
        grid2, time = history_p[i]
        state, time = states[i]
        # Render grid
        fig_grid1.matshow(grid1, vmin=-1, vmax=1, cmap='jet')
        fig_grid2.matshow(grid2, vmin=-1, vmax=1, cmap='jet')
        fig_grid3.matshow(state, vmin=-1, vmax=1, cmap='jet')
...
def trajectory_empty(pos0, game, T = 50, T0 = 100, Time_stop = 60, reward_control = 0, action_ = 0, epsilon = 0, open_loop = True):
...
#     print (game.visible_state)
    ...
    while not done:
        if open_loop == True:
            done, action = game.step_empty(stim = stim, action_ = action_, epsilon = epsilon, T = T, T0 = T0, time_stop = Time_stop, open_loop = open_loop) # Down
        else:
            done, action = game.step_empty(stim = stim, action_ = action, epsilon = epsilon, T = T, T0 = T0, time_stop = Time_stop, open_loop = open_loop) # Down
        # up 
        if action == 0: y -= 1
        # right
        elif action == 1: x += 1
