...
def Memory(weight, k_action = 1, k_stim = 1, k_internal = 1, epsilon = 1, context_gain = 1, num_trials = 2):
    ...
    for pos_reward in [(9, 5), (9, 13)]:
        ...
        pca.Dynamics(Actions = num_trials * [0, 1, 2, 3], legend = True, T_total = 200, T_stim = 30, T_duration = 3, \
...
def Memory(weight, k_action = 1, k_stim = 1, k_internal = 1, epsilon = 1, context_gain = 1, num_trials = 32):
    # reference from net 1 
    def placefield(pos): 
        field =np.zeros((2, 19))
        for k in range(2):
