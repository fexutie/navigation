...
class RNN(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, inertia = 0.5, k_action = 1, max_size = 20, ranks = 5):
        super(RNN, self).__init__()
        self.hidden_size = hidden_size
        self.output_size = output_size
        self.i2h = nn.Parameter(torch.randn(input_size, hidden_size) * 10 * np.sqrt(2.0/(input_size + hidden_size)))
        self.W0 = nn.Parameter(torch.randn(hidden_size, hidden_size) * 1.0 * np.sqrt(2.0/hidden_size))
        self.S = nn.Parameter(torch.randn(ranks)* 1.0/hidden_size) 
        self.U = nn.Parameter(torch.randn(hidden_size, ranks))
        self.V = nn.Parameter(torch.randn(ranks, hidden_size))
    ...
    def forward(self, input, hidden, action, reward):
        ...
        hidden_ = torch.tanh(input.matmul(self.i2h) + hidden.matmul(self.W0 + self.U.matmul(torch.diag(self.S)).matmul(self.V)) + self.k_action * action.matmul(self.a2h) + reward.matmul(self.r2h) + self.bh)
