...
class MultipleTasks():
    def __init__(self, weight_write, task =  'basic', noise = 0.0, weight1 = None, weight2 = None):
        if task == 'basic':
            self.game = CreateGame(GameBasic, holes= 0, noise = noise, task = task, weight1 = weight1, weight2 = weight2)
        if task == 'hole':
            self.game = CreateGame(GameHole, holes= 50, noise = noise, task = task, weight1 = weight1, weight2 = weight2)
        if task == 'bar':
            self.game = CreateGame(GameBar, holes= 0, noise = noise, task = task, weight1 = weight1, weight2 = weight2)
        if task == 'scale':
            self.game = CreateGame(GameScale, holes=0, noise = noise, task = task, weight1 = weight1, weight2 = weight2)
        if task == 'scale_x':
            self.game = CreateGame(GameScale_x, holes=0, noise = noise, task = task, weight1 = weight1, weight2 = weight2)
        if task == 'scale_y':
            self.game = CreateGame(GameScale_y, holes=0, noise = noise, task = task, weight1 = weight1, weight2 = weight2)
        if task == 'implicit':
            self.game = CreateGame(GameImplicit, holes=0, noise = noise, implicit = True, weight1 = weight1, weight2 = weight2)
...
#       need to take the state dict as a new dict for updating 
    ...
    def qlearn(self, task, weight_read, weight_write, episodes = 10, save = True, size_train =  [15], \
        size_test = [15], test_only = False, noise = 0.0, k_action = 1, iterations = 50, epochs = 10):
#         self.game.net.load_state_dict(torch.load(weight_read))
#         if h2o == True:
#             self.game.net.h2o = nn.Parameter(torch.randn(1024, 4) * 0.01 * np.sqrt(2.0/(1024 + 4)))
        self.game.net.k_action = k_action    
        e_rate = [noise for r in range(episodes)]
        rls_q1 = RLS(1e2, lam = 0.5)
        rls_q2 = RLS(1e2, lam = 0.5)
        rls_meta = RLS(1e2, lam = 1)
        # q leanring phase
...
#             self.game.seed_range = 2 + (n//10) * 10
            ...
            if test_only == False:
                self.game.experiment(rls_q1, rls_q2, rls_meta, iterations = iterations, epochs = epochs, epsilon = e, size_range = size_train)
                if save == True:
            ...
            if task == 'bar':
                rls_q1.lam = np.sqrt((rewards + 1)/2)
                rls_q2.lam = np.sqrt((rewards + 1)/2)
            else:
                rls_q1.lam = 0.5
                rls_q2.lam = 0.5
