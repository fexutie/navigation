...
def Test(task, game, weight = 0, size = 15, test_size = 0, limit_set = 8, step = 1):
    if weight != 0:
    ...
    def TestScale(game, reward_control= 0, cross=False, size = 50, test = test_size, limit_set = limit_set, step = step):
        Rewards = 0
        iters = 0
        error = 0
        for j in np.arange(2 * VISIBLE_RADIUS, size + 2 * VISIBLE_RADIUS, step):
