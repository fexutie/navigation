...
class RNN(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, inertia = 0.5, k_action = 0, max_size = 20):
        ...
        self.a2h = nn.Parameter(torch.randn(4, hidden_size) * 10 * np.sqrt(2.0/(hidden_size + 4)))
...
class WallDecoder(nn.Module):
    ...
    def crossentropyloss(self, predict, target, batch_size = 4):
#         print (predict, target)
        return torch.mean(- F.softmax(target.view(batch_size,-1), dim = 1) \
                        * torch.log(F.softmax(predict.view(batch_size,-1), dim = 1) + 1e-5)) 