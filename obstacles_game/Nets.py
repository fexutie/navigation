...
class PretrainTest():
    def __init__(self, weight_write, holes = 0, inputs_type = (1, 0), action_coupling = 1):
        ...
        self.game = ValueMaxGame(grid_size = (15, 15), holes = holes, random_seed = 4 , set_reward = [(0.5, 0.25), (0.5, 0.75)], input_type = inputs_type[1], action_coupling = action_coupling)
...
#       need to take the state dict as a new dict for updating 
    ...
    def decode_wall(self, weight = None, size_range = [15], size_test = [15], epsilon = 0, epochs = 100):
        if weight != None:
            self.game.net.load_state_dict(torch.load(weight))
        else:
            self.game.net.load_state_dict(torch.load(self.weight))
        Prec = 0
        # iterations are number of turns for decoder update, epochs are how many turns of games at each update  
        self.game.episode(epochs = epochs, epsilon = epsilon, reward_control = 0, size_range = size_range, prob = np.ones(len(size_range))/len(size_range), train_hidden = False, test = False, decode = True)
        hiddens = self.game.Hiddens_batch
        poss = self.game.Pos_batch 
        def stimulus_train(Z):
            Stim = []
            stim = 0
            for z in Z:
                z = z[0]
                ...
                Stim.append(stim)
            return Stim
        Stim = stimulus_train(poss)
#         print(Stim[:10], poss[:10])
        Data = [(h.data.view(-1), torch.eye(5)[s].view(-1)) for (h, s) in zip(hiddens, Stim)] 
        # train decoder
        net = WallDecoder(512, 5)
        net.cuda()
...
#         print (Data[0][1])
    ...
    #             print (inputs, labels)
                ...
                outputs = net(inputs)
                loss = net.crossentropyloss(outputs, labels, batch_size = len(labels))
                loss.backward()
                optimizer.step()
                # print statistics
    ...
    def dec_wall_test(self, size_range = [15], size_test = [15], epsilon = 1, epochs = 10):
        self.game.episode(epochs = epochs, epsilon = epsilon, reward_control = 0, size_range = size_range, prob = np.ones(len(size_range))/len(size_range), train_hidden = False, test = False, decode = True)
        hiddens = self.game.Hiddens_batch
        poss = self.game.Pos_batch 
        def stimulus_train(Z):
            ...
            for z in Z:
                ...
                Stim.append(stim)
            return Stim
        self.Stim = stimulus_train(poss)
        Data = [(h.data.view(-1), torch.eye(5)[s].view(-1)) for (h, s) in zip(hiddens, self.Stim)] 
        # train decoder
        net = WallDecoder(512, 5)
        net.cuda()
#         Data = torch.FloatTensor(Data)
#         print (Data[0][1])
        testloader = torch.utils.data.DataLoader(Data, batch_size=4,
                                          shuffle=True, num_workers=2)
        self.Real = []
        self.Predict = []
        for i, data in enumerate(testloader):
            ...
            labels = labels.cuda()
            outputs = net(inputs)
            self.Predict.append(np.argmax(outputs.cpu().data.numpy(), axis = 0))
            self.Real.append(np.argmax(labels.cpu().data.numpy(), axis = 0))
    ...
    def qlearn(self, weight_read, weight_write, iterations = 5, save = True, size_train = np.arange(10, 51, 10), \
        ...
        self.game.net.k_action = self.action_coupling
        if h2o == True:
        ...
        for n,e in enumerate(e_rate):
            ...
            if test_only == False:
                self.game.experiment(rls_q, rls_sl, iterations = 5, epochs= 200, epsilon = e, size_range = size_train)    
                if save == True:
    ...
    def TestAllSizes(self, size_range = np.arange(15, 86, 10), limit_set = 8, test_size = None, seed = 0):
        self.game.net.load_state_dict(torch.load(self.weight))
        self.Performance = []
        for size in size_range:
            if test_size == None: 
                ...
                test_size = test_size
            Rewards0 = Test(self.game, reward_control = 0, size = size, test = test_size, limit_set = limit_set, seed = seed)
            Rewards1 = Test(self.game, reward_control = 1, size = size, test = test_size, limit_set = limit_set, seed = seed)
...
def trajectory(game, pos0, reward_control = 0, init_hidden = True, hidden = torch.zeros(1, 512), size = 19, test = 2, map_set = [], Input_split = False, limit_set = 4, epsilon = 0):
    ...
    while not done:
        ...
        hidden0 = game.hidden.clone()
        if Input_split == True:
            Input_h.append(game.hidden.matmul(game.net.h2h).data.numpy())
            Input_i.append(torch.FloatTensor(state).resize(1,9).matmul(game.net.i2h).data.numpy())
            Input_a.append(game.action.matmul(game.net.a2h).data.numpy())
    if Input_split == False:
        return Pos, np.array(Hidden), np.array(dH), np.array(Action), State, reward
    else:
        return Pos, np.array(Hidden), np.array(dH), np.array(Action), State, reward, Input_h, Input_i, Input_a 
...
class ActivityCheck():
    def __init__(self, size = 15, num_pc = 3, holes = 50):
    ...
    def activity(self, reward_control = 0, weight = 0, pc = False, vects = None, noise = 0.1):    
        game = ValueMaxGame(grid_size = (self.size, self.size), holes = 0, random_seed = 4 , set_reward = [], input_type = 0, action_coupling = 1, discount = 0.9, alpha = 1, time_limit = 8 * self.size, lam = 0.)
